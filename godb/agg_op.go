...
func extractGroupByKeyTuple(a *Aggregator, t *Tuple) (*Tuple, error) {
	vals := make([]DBValue, 0, len(a.groupByFields))
	...
	var fields = make([]FieldType, 0, len(a.groupByFields))
	for _, g := range a.groupByFields {
		fields = append(fields, g.GetExprType())
	}

	return &Tuple{TupleDesc{fields}, vals, 0}, nil
}
...
func getFinalizedTuplesIterator(a *Aggregator, groupByList []*Tuple, aggState map[any]*[]AggState) func() (*Tuple, error) {
	...
	return func() (*Tuple, error) {
		...
		aggState := aggState[GbyTuple.tupleKey()]

		tuple := GbyTuple
		for _, s := range *aggState {
			if tuple == nil {
				tuple = s.Finalize()
			} else {
				tuple = joinTuples(tuple, s.Finalize())
			}
		}
